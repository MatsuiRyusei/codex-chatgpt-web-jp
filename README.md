...
> [!NOTE]
> このリポジトリは [miuuyy/codex-chatgpt-web](https://github.com/miuuyy/codex-chatgpt-web) をベースにした日本語版です。
> 上流 **5.0.6**（`e85e369`、2026-09-08）に同期し、元プロジェクトの MIT License と著作権表示を引き継いでいます。
> 以下のインストーラーは上流のリリースを使用します。この日本語版を実行する場合は、**ソースから実行**を参照してください。
...
> [!TIP]
> 元プロジェクトの作者は **[ChatGPT Persona Voice](https://github.com/miuuyy/ChatGPT-Persona-Voice)** も開発しています。
...
| **Full harness（自動操作あり）** | Free/Go: Luna、Plus: Instant～High、Pro: Extra High と Pro を追加 | Pro を含むすべての表示 effort で使用可能 | OpenAI トンネル + ChatGPT コネクタ |
| **Zero Risk** | ChatGPT のモデルと effort を手動で選択。Pro サイズのコンテキストも選択可能 | 使用可能。ターンに紐付いた完全な Codex ハーネスを維持 | 専用の OpenAI トンネル + `Codex Zero Risk` コネクタ。貼り付けと送信は手動 |

自動モードのモデル選択画面では、各項目が 1 つの固定 ChatGPT モードに対応します。Codex には内蔵の Effort と Speed 行も表示されますが、
...
Zero Risk はローカル Responses ブリッジと完全な Codex ハーネスを維持しますが、ChatGPT ページの
読み取り・変更やプロンプトの自動送信は行いません。ランチャーがプロンプトを準備してコピーし、
利用者がモデル、effort、`Codex Zero Risk` コネクタを選び、自分で貼り付けて送信します。
これにより、ChatGPT のブラウザー自動操作に由来するアカウントリスクを避けます。
...
ランチャーの **MCP** ページが設定全体を案内します。具体的な操作は
[動画付きの手順](TROUBLESHOOTING.md)を参照してください。
