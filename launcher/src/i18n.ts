...
const ja: Record<keyof typeof en, string> = {
  product: "Codex Web GPT",
  devBadge: "DEV",
  devSetupTitle: "隔離された DEV プロファイルを設定",
  devSetupSubtitle: "このブラウザー、アカウント、設定、ランタイムデータは通常のランチャーと Codex から分離されます。",
  devCoreSetup: "DEV プロファイル",
  devStepInstall: "DEV ハーネスを初期化",
  devStepInstallBody: "隔離された DEV ホームにアカウント機能を保存します。Codex ルート、Responses リスナー、システムサービスはインストールされません。Full モードは隔離された MCP トンネルのみを管理します。",
  devInstall: "プロファイルを初期化",
  devReinstall: "プロファイルを更新",
  devMcpTitle: "MCP 経由のシミュレーションツール",
  devMcpSubtitle: "この別の ChatGPT アカウントを隔離されたリポジトリハーネスに接続します。",
  devMcpBody: "DEV ランチャーは隔離された MCP トンネルを待機状態に保ちます。名前付きリポジトリチャットはそこへ接続し、ツール操作に対して明示的なシミュレーション結果を返します。",
  devConnectorIsolationNotice: "下記の正確な DEV 名で別のコネクタを作成してください。Codex Native2 は変更せず、本番利用とリポジトリ開発を並行して実行できるようにします。",
  devSettingsTitle: "DEV プロファイル設定",
  devKeepRunningBody: "ウィンドウを閉じても、隔離されたブラウザーセッションと DEV MCP トンネルを利用可能な状態に保ちます。",
  biggerContext: "Bigger Context（実験的）",
  biggerContextBody: "警告：小さいターンは1件のメッセージとして保持し、大きなコンテキストは2～3件のメッセージに分割します。最後の部分で追加リクエストなしにタスクを開始します。モデルのコンテキストとコンパクション閾値は3倍になります。変更後は Codex を再起動してください。追加リクエストによりレート制限や一時的なクールダウンが増える場合があります。既定では無効です。",
  biggerContextRecommendationTitle: "最大3倍のコンテキストを使用",
  biggerContextRecommendationBody: "Bigger Context は最近のビルドで十分にテストされており、ChatGPT Web で最大3倍のコンテキストを利用できます。まだ実験的な機能なので、問題があれば設定から再び無効にできます。",
  biggerContextRecommendationToggleBody: "大きなタスクを複数メッセージに分け、モデルのコンテキスト上限とコンパクション上限を引き上げます。",
  tagline: "ネイティブ Codex ハーネス内で ChatGPT Web を利用",
  chooseLanguage: "言語を選択",
  chooseLanguageHint: "後から「設定」で変更できます。",
  english: "English",
  chinese: "简体中文",
  continue: "続ける",
  supportTitle: "開始する前に",
  supportBody: "リポジトリに Star を付け、X プロフィールを開いて、より多くの開発者に Codex Web GPT を知ってもらえるようご協力ください。",
  star: "GitHub で Star",
  ...
  hideTab: "タブを閉じる",
  browserTabLimit: "ChatGPT Web タブは同時に最大5つまで開けます。この上限により、ChatGPT アカウントへの過度な並列通信を防ぎます。",
  ...
  setupSubtitle: "3つの確認を完了すると、ネイティブ Codex のモデル選択画面で ChatGPT Web を利用できます。",
  coreSetup: "コアセットアップ",
  optional: "任意",
  required: "必須",
  stepAccount: "ChatGPT にログイン",
  stepAccountBody: "内蔵 ChatGPT ブラウザーから直接ログインします。ログイン情報はこのランチャー専用のプライベートプロファイルに保持されます。",
  signIn: "ログインを開く",
  checkingSignIn: "保存済みセッションを確認中",
  verifySignIn: "ログイン状態を確認",
  signedIn: "ログイン済み",
  stepSmoke: "ブラウザーのスモークテストを実行",
  stepSmokeBody: "High を選択し、短い一時メッセージを送信して、ストリーミング応答が最後まで完了することを確認します。",
  runSmoke: "スモークテストを実行",
  smokePassed: "スモークテスト成功",
  stepInstall: "Codex にインストール",
  stepInstallBody: "Codex のネイティブモデル一覧を置き換えずに ChatGPT Web モデルを追加します。現在のカスタムルートは保存され、ブリッジを無効にすると復元されます。",
  install: "モデルをインストール",
  reinstall: "Codex セットアップを修復",
  awaitingCodex: "Codex を再起動",
  restartCodex: "モデル一覧を更新するため、Codex を一度再起動してください。",
  mcpTitle: "MCP 経由でネイティブ Codex ツールを利用",
  mcpSubtitle: "OpenAI トンネル経由で ChatGPT をアクティブな Codex ハーネスに接続します。",
  mcpBody: "Pro を含む利用可能なすべての ChatGPT Web effort から、アクティブな Codex ハーネスを利用できるようにします。",
  ...
  mcpStepOneBody: "OpenAI トンネルを作成して Tunnel ID をコピーし、Tunnels Read + Use 権限を持つ通常の API キーを作成します（無料。キーはトンネル実行のみに必要です）。（ChatGPT ワークスペースの作成も忘れないでください。）",
  openTunnels: "Tunnels を開く",
  openKeys: "API キーを作成",
  mcpStepTwo: "ローカルハーネスを接続",
  mcpStepTwoBody: "Tunnel ID と API キーを貼り付けます。トンネルは ChatGPT プラグインを利用する OpenAI アカウントと同じアカウントに属している必要があります。キーは端末内のプライベートストレージにのみ保存され、ランチャーログには書き込まれません。",
  mcpStepTwoHint: "この手順が成功し、トンネルが動作してから ChatGPT に MCP コネクタを追加できます。",
  mcpCatalogRequired: "Codex モデルのインストールと検証が完了するまでハーネスには接続できません。「セットアップ」に戻って「モデルをインストール」をクリックし、Codex を一度再起動して、モデルカタログの確認が完了するまで待ってください。",
  tunnelId: "Tunnel ID",
  runtimeKey: "API キー（Admin キーではありません）",
  connect: "ハーネスを接続",
  reconnect: "ハーネスを再接続",
  credentialsConfigured: "トンネル認証情報を保存済み",
  credentialsConfiguredBody: "この端末にプライベート保存されている Tunnel ID と API キーをランチャーが再利用します。",
  replaceCredentials: "認証情報を置き換える",
  keepCredentials: "保存済み認証情報を使う",
  mcpStepThree: "ChatGPT コネクタを接続",
  mcpStepThreeBody: "コネクタを作成する前に ChatGPT の設定で Developer Mode を有効にします。ChatGPT Plugins で新しいコネクタを作成し、Tunnel を選択して、作成したトンネルを指定します。Authentication は None に設定し、下に表示されるコネクタ名を正確に使用してください。Permissions を開き「Allow all actions」を選択します。「Allow low-risk actions」ではコマンドやパッチ呼び出しがこのランタイムに届く前にブロックされます。外側の Codex ハーネスでは引き続きサンドボックスと承認が適用されます。最後にランタイムを検証します。",
  connectorMigrationNotice: "Codex Native からアップグレードする場合は、古いコネクタをそのまま残し、Codex Native2 を新しいコネクタとして作成してください。Codex Native の名前変更や更新はしないでください。旧コネクタしかない構成はランタイム検証で拒否されます。",
  openConnectors: "ChatGPT Plugins を開く",
  connectorName: "コネクタ名",
  verifyRuntime: "ランタイムを検証",
  activityTitle: "ランタイムアクティビティ",
  activitySubtitle: "ローカル診断情報です。共有前にプライバシー保護済みのコピーを書き出してください。生ログはこの端末にのみ保存されます。",
  recentActivity: "最近のイベント",
  noLogs: "ランタイムイベントはまだありません。",
  exportSafeLog: "安全なログを書き出す",
  settingsTitle: "ランチャー設定",
  general: "一般",
  launchAtLogin: "ログイン時に起動",
  launchAtLoginBody: "Codex を開く前からローカル Responses ルートを利用できる状態にします。",
  keepRunningOnClose: "ウィンドウを閉じてもサーバーを実行",
  keepRunningOnCloseBody: "ランチャーをシステムトレイに隠し、ネイティブモデルと ChatGPT Web モデルを引き続き利用できるようにします。",
  showDuringTurns: "ターン中にブラウザーを表示",
  showDuringTurnsBody: "ブラウザーターンがアクティブなとき、内蔵 ChatGPT 画面を表示します。",
  language: "言語",
  diagnostics: "診断",
  runDoctor: "診断を実行",
  cancelTurns: "アクティブな Codex ターンをキャンセル",
  cancelTurnsBody: "アクティブな HTTP ストリームと、それに保持されている ChatGPT ブラウザーターンを中止します。",
  turnsCancelled: "アクティブな Codex ターンをキャンセルしました",
  uninstallIntegration: "Codex 連携を削除",
  uninstallIntegrationBody: "以前の Codex モデルルートを復元し、プライベートブリッジランタイムを削除します。",
  integrationRemoved: "連携を削除しました。Codex を一度再起動してください",
  running: "実行中",
  complete: "完了",
  failed: "失敗",
  healthy: "正常",
  ...
  error: "エラーが発生しました",
  dismiss: "閉じる",
  sessionReminderTitle: "ChatGPT セッションを更新",
  sessionReminderBody: "2日ごとの再ログインを推奨します。古い ChatGPT セッションは Web モデルのターンや MCP ツールを中断することがあります。",
  logOut: "ログアウト",
};
