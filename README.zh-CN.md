...
> [!NOTE]
> 本仓库是 [miuuyy/codex-chatgpt-web](https://github.com/miuuyy/codex-chatgpt-web) 的日语本地化分支。
> 已同步上游 **5.0.6**（`e85e369`，2026-09-08），保留原项目的 MIT License 和版权声明。
> 下方安装命令使用上游发行版；运行本分支请参阅**从源码运行**。
...
> [!TIP]
> 上游作者还开发了 **[ChatGPT Persona Voice](https://github.com/miuuyy/ChatGPT-Persona-Voice)**：一款
...
| **完整 harness（自动操作）** | Free/Go：Luna；Plus：Instant–High；Pro：增加 Extra High 和 Pro | 每个列出的 effort 均支持，包括 Pro | OpenAI 隧道 + ChatGPT 连接器 |
| **Zero Risk** | 手动选择 ChatGPT 模型和 effort；可选 Pro 大小的上下文 | 可用；保留完整的回合绑定 Codex harness | 独立的 OpenAI 隧道 + `Codex Zero Risk` 连接器；手动粘贴并发送 |

自动模式选择器中的每一项都对应一个固定的 ChatGPT 模式。Codex 仍会显示内置的 Effort 和 Speed
选项，但更改它们不会在后台静默切换所选的浏览器模型。在完整模式下，每一个可用 effort 都会
获得同一个与当前回合绑定的 MCP 能力；Pro 没有单独限制，也没有缩减后的工具契约。

Zero Risk 保留本地 Responses 桥接程序和完整 Codex harness，但不会读取或更改 ChatGPT 页面，
也不会自动发送提示。启动器准备并复制提示；你自行选择模型、effort 和 `Codex Zero Risk` 连接器，
然后手动粘贴并发送，从而避免 ChatGPT 网页自动操作本身带来的账户风险。
...
启动器的 **MCP** 页面会引导完整设置。具体操作请参阅[视频指南](TROUBLESHOOTING.md)。
